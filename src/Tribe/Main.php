...
if ( ! class_exists( 'Tribe__Events__Cleanup__Main' ) ) {
	class Tribe__Events__Cleanup__Main {
		...
		protected function looks_tribal( $string ) {
			...
			if ( false === strpos( $string, 'tribe' ) ) {
				return false;
			}
			if ( false !== strpos( $string, 'event' ) ) {
				return true;
			}
			if ( false !== strpos( $string, 'venue' ) ) {
				return true;
			}
			if ( false !== strpos( $string, 'organizer' ) ) {
				return true;
			}
		...
		public function admin_screen() {
			$this->basic_checks();
			$plugin = $this;
			include $this->plugin_dir . 'src/views/screen.php';
		}

		public function enqueue_resources() {
			wp_enqueue_script( 'tribe-events-cleanup', $this->plugin_url . 'src/resources/js/tribe-events-cleanup.js', 'jquery', false, true );
			wp_enqueue_style( 'tribe-events-cleanup', $this->plugin_url . 'src/resources/css/tribe-events-cleanup.css' );
		}

		public function do_cleanup() {
			if ( ! $this->pre_cleanup_checks() ) {
				return;
			}
		...
		protected function keep_cleaning() {
			...
			if ( $this->counts->events > 0 ) {
				$this->remove_posts( self::EVENTS );
			} elseif ( $this->counts->venues > 0 ) {
				$this->remove_posts( self::VENUES );
			} elseif ( $this->counts->organizers > 0 ) {
				$this->remove_posts( self::ORGANIZERS );
			} elseif ( $this->counts->categories > 0 ) {
				$this->remove_terms();
			} elseif ( $this->counts->capabilities > 0 ) {
				$this->remove_caps();
			} elseif ( $this->counts->options > 0 ) {
				$this->clean_options();
			} else {
